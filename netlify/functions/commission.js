...
// Bonus commission: for every N priced rooms booked (unified roster, no contract
// split), the group earns an additional flat bonus. Only these three ever share in
...
export default async (req) => {
  ...
  order.forEach((key) => {
    ...
    // Bonus room tally — every priced room counts toward the group's bonus
    // threshold, including an agent's own personal room. Unified across the whole
    // roster; there's no separate contract split anymore. (This is deliberately
    // different from "Agent stake in the trip," which is computed independently,
    // client-side.)
